 ...
 * 2015-08-30 2.2 AG coverart module (Andreas Goetz)
 * 2016-02-27 2.5 TC rewrite for pre-3.0
 * 2016-06-07 2.6 TC moodeOS 1.0
 * 2016-08-28 2.7 TC
 * - add pkgdate to About
 * - fix wrong encodedAT rate displayed for UPnP files
 * 2016-11-27 3.0 TC
 * - improve chip option handling
 * - add squeezelite indicator
 * - improve resiliency to connection interruptions in engineMPD() error handling
 * 2016-12-05 3.1 TC
 * - fix PCM5121 chip option display
 * - fix scrollTo on Browse panel
 *
 */
...
var UI = {
	...
	lastTitle: 'fxdnjkfw',
	defCover: 'images/default-cover.jpg',
	knobPainted: false,
	pcm5122Dev: false,
	devSettings: '',
...
function sendMoodeCmd(type, cmd, data, async) {	
	...
	$.ajax({
		...
		error: function() {
			console.log(cmd + ' no data returned');
			obj = false;
		}
	});
...
// mpd metadata engine
function engineMpd() {
	//console.log('engineMpd: state=(' + MPD.json['state'] + ')');
    $.ajax({
		type: 'GET',
		...
		cache: false,
		success: function(data) {
			//console.log('engineMpd: success function() data=(' + data + ')');
			MPD.json = JSON.parse(data);

			if (typeof(MPD.json['error']) === 'undefined') {
				$('#reconnect').hide();
				//console.log('engineMpd: idle_timeout_event=(' + MPD.json['idle_timeout_event'] + ')');
				...
				setTimeout(function() {
					// socket error
					// can occur during reboot/powerup if client connects before mpd started by worker
					//console.log('engineMpd: socket error=(' + MPD.json['error'] + '), module=(' + MPD.json['module'] + ')');
					engineMpd();
				}, 3000);
			}
		},
		error: function(data) {
			//console.log('engineMpd: error function() data=(' + JSON.stringify(data) + ')');
...
function renderReconnect() {
	if (UI.turnOff) {
		$('#bootready').html(UI.bootTicker += '<i class="icon-stop"> </i>'); // #reboot screen
	} else {
		$('#reconnect').show(); 
	}
...
function renderUI() {
	...
	if (SESSION.json['airplayactv'] != '1') {
		...
		if (MPD.json['title'] !== UI.lastTitle) { // prevent unnecessary image reloads (clicking on same item)
		...
		if (SESSION.json['xtagdisp'] === 'Yes') {
			...
			extraTags += MPD.json['composer'] ? '&nbsp&bull; ' + MPD.json['composer'] : '';
			
			if (MPD.json['artist'] === 'Radio station') {
				extraTags += MPD.json['bitrate'] ? '&nbsp&bull; ' + MPD.json['bitrate'] : '';
			} else {				
	...
	if (SESSION.json['ashufflesvc'] === '1') {
		SESSION.json['ashuffle'] ==='1' ? $('#random').addClass('btn-primary') : $('#random').removeClass('btn-primary');
	} else {
...
function renderPlaylist() {
	// turn playlist off if random play through huge pl in browser causes performance issue
    ...
    $.getJSON('command/moode.php?cmd=playlist', function(data) {
        ...
        if (data) {
            for (i = 0; i < data.length; i++) {
				...
				// action menu
				output += '<div class="pl-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-playlist-item"><i class="icon-reorder"></i></a></div>';
				...
				// radio station
	                ...
	                // line 1 title
	                output += '<div class="pl-entry">';
	                // use 'Streaming source' string for Classic And Jazz station
	                if (typeof(data[i].Title) === 'undefined' || data[i].Title.trim() == '' || data[i].file == 'http://classicandjazz.ice.infomaniak.ch:80/classicandjazz-128.mp3') {
...
function parseMpdResp(data, path, i) {
	...
	if (typeof data[i].file != 'undefined') {
		...
		if (typeof data[i].Title != 'undefined') {
			...
			if(data[i].file.substr(0, 5) == "RADIO") {
				output += '"><div class="db-icon db-song db-browse"><i class="icon-microphone sx db-browse"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-radio-item"><i class="icon-reorder"></i></a></div><div class="db-entry db-song db-browse">';
				itemType = "Radio station";
			// cue sheet, use song file action menu
			} else if (fileExt == "cue") {
				output += '"><div class="db-icon db-song db-browse"><i class="icon-list-ul icon-root sx"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-folder-item"><i class="icon-reorder"></i></a></div><div class="db-entry db-song db-browse">';
				itemType = "Cue sheet";
			} else {
				// different icon and file type text
				if (data[i].file.substr(0,4) == "http") {
					output += '"><div class="db-icon db-song db-browse"><i class="icon-microphone sx db-browse"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-savedpl-item"><i class="icon-reorder"></i></a></div><div class="db-entry db-song db-browse">';
					itemType = "Radio station";
				} else {
					output += '"><div class="db-icon db-song db-browse"><i class="icon-music sx db-browse"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-savedpl-item"><i class="icon-reorder"></i></a></div><div class="db-entry db-song db-browse">';
					itemType = "Song file";
				}
			}
	...
	// handle saved playlist 
		...
		if (data[i].playlist.substr(data[i].playlist.lastIndexOf('.') + 1).toLowerCase() == 'wv') {
			...
			output += '"><div class="db-icon db-folder db-browse"><i class="icon-list-ul icon-root sx db-browse"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-savedpl-root"><i class="icon-reorder"></i></a></div><div class="db-entry db-savedplaylist db-browse">';
			output += data[i].playlist;
			output += '</div></li>';
		}
	} else {
		...
		if (path != '') {
			output += '"><div class="db-icon db-folder db-browse"><i class="icon-folder-open sx"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu"><i class="icon-reorder"></i></a></div><div class="db-entry db-folder db-browse">';
		} else {
			output += '"><div class="db-icon db-folder db-browse"><i class="icon-hdd icon-root sx"></i></div><div class="db-action"><a class="btn" href="#notarget" title="Actions" data-toggle="context" data-target="#context-menu-root"><i class="icon-reorder"></i></a></div><div class="db-entry db-folder db-browse">';
		}
...
function setVolume(level, event) {
	...
	if (SESSION.json['volmute'] == '0') { // unmuted, set volume (incl 0 vol)
		...
		if (SESSION.json['mpdmixer'] == 'hardware' && SESSION.json['volcurve'] == 'Yes') {
			// HWvol = a x log10(volume) -2a + 100, where a = 56
			var maxPct = parseInt(SESSION.json['volmaxpct']) * .01; // cap max volume level, default is 100%
			var curveFactor = parseInt(SESSION.json['volcurvefac']); // adjust curve to be more or less aggressive
			if (level > 1) {
				level = Math.floor((curveFactor * (Math.log(level) / Math.LN10)) - (2 * curveFactor) + 100); // round down
				level = Math.round(level * maxPct); // round up
			}
		}
		//console.log('output level=', level)
		$('#output-level').text('mpd level= ' + level.toString()); // debug on playback panel

		// negative values occur when curveFactor > 56
		if (level < 0) {level = 0;}
		...
		if (level == 0 && event == 'mute')	{
			...
			SESSION.json['volknob'] = level.toString(); // muted, just store the volume for display
		}
...
$('.context-menu a').click(function(){
    ...
    if ($(this).data('cmd') == 'customize') {
		...
		$('#playlist-display span').text(SESSION.json['pldisp']);
		$('#extratag-display span').text(SESSION.json['xtagdisp']);
		$('#resume-aftersps span').text(SESSION.json['rsmaftersps']);

		// hardware volume control
		$('#logarithmic-curve-enabled span').text(SESSION.json['volcurve']);
		
		$('#volume-curve-list').html(		
		...
		if (SESSION.json['volcurvefac'] == 56) {$('#volume-curve-factor span').text("Standard");}
		else if (SESSION.json['volcurvefac'] == 66) {$('#volume-curve-factor span').text("Less (-10)");}
		else if (SESSION.json['volcurvefac'] == 76) {$('#volume-curve-factor span').text("Less (-20)");}
		else if (SESSION.json['volcurvefac'] == 86) {$('#volume-curve-factor span').text("Less (-30)");}
		else if (SESSION.json['volcurvefac'] == 50) {$('#volume-curve-factor span').text("More (+06)");}
		else if (SESSION.json['volcurvefac'] == 44) {$('#volume-curve-factor span').text("More (+12)");}
		else if (SESSION.json['volcurvefac'] == 38) {$('#volume-curve-factor span').text("More (+18)");}
		
		$('#volume-max-percent').val(SESSION.json['volmaxpct']);

		if (SESSION.json['mpdmixer'] != 'hardware') {
			$('#volcurve').attr('disabled', 'true');
			$('#volcurvefac').attr('disabled', 'true');
			$('#volume-max-percent').attr('disabled', 'true');
		}

		// audio device options
		var obj = sendMoodeCmd('POST', 'readaudiodev', {'name': SESSION.json['i2sdevice']});

		if (obj != null  && obj['settings'] != null && (obj['dacchip'] == 'Burr Brown PCM5242' || obj['dacchip'] == 'Burr Brown PCM5142' || obj['dacchip'] == 'Burr Brown PCM5122' || obj['dacchip'] == 'Burr Brown PCM5121' || obj['dacchip'] == 'Burr Brown PCM5122 (PCM5121)' || obj['dacchip'] == 'Burr Brown TAS5756')) {
			UI.pcm5122Dev = true;
			UI.devSettings = obj['settings']; 
			var settings = obj['settings'].split(',');

			$('#pcm5122-analog-vol span').text(settings[0] === '100' ? '0 dB' : '-6 dB'); // Analog volume
			$('#pcm5122-analog-pbb span').text(settings[1] === '100' ? '.8 dB' : '0 dB'); // Analog playback boost
			$('#pcm5122-filter-name span').text(settings[2]); // Digital interpolation filter

			$('#pcm5122-filter-list').html(		
				'<li><a href="#notarget" data-cmd="pcm5122-filter-name"><span class="text">FIR interpolation with de-emphasis</span></a></li>' +
				'<li><a href="#notarget" data-cmd="pcm5122-filter-name"><span class="text">High attenuation with de-emphasis</span></a></li>' +
				'<li><a href="#notarget" data-cmd="pcm5122-filter-name"><span class="text">Low latency IIR with de-emphasis</span></a></li>' +
				'<li><a href="#notarget" data-cmd="pcm5122-filter-name"><span class="text">Ringing-less low latency FIR</span></a></li>'
			);
	
			$('#pcm5122-analog-vol-list').html(		
				'<li><a href="#notarget" data-cmd="pcm5122-analog-vol"><span class="text">0 dB</span></a></li>' +
				'<li><a href="#notarget" data-cmd="pcm5122-analog-vol"><span class="text">-6 dB</span></a></li>'
			);
	
			$('#pcm5122-analog-pbb-list').html(		
				'<li><a href="#notarget" data-cmd="pcm5122-analog-pbb"><span class="text">.8 dB</span></a></li>' +
				'<li><a href="#notarget" data-cmd="pcm5122-analog-pbb"><span class="text">0 dB</span></a></li>'
			);
		} else {
			UI.pcm5122Dev = false;
			$('#pcm5122-filtername').attr('disabled', 'true');
			$('#pcm5122-analogvol').attr('disabled', 'true');
			$('#pcm5122-analogpbb').attr('disabled', 'true');
		}
...
$('.btn-customize-update').click(function(){
	...
	if (SESSION.json['themecolor'] != $('#theme-color span').text()) { // detect theme change
	...
	SESSION.json['pldisp'] = $('#playlist-display span').text();
	SESSION.json['xtagdisp'] = $('#extratag-display span').text();
	SESSION.json['rsmaftersps'] = $('#resume-aftersps span').text();

	// hardware volume control
	SESSION.json['volcurve'] = $('#logarithmic-curve-enabled span').text();
	if ($('#volume-curve-factor span').text() == "Standard") {SESSION.json['volcurvefac'] = 56;}
	else if ($('#volume-curve-factor span').text() == "Less (-10)") {SESSION.json['volcurvefac'] = 66;}
	else if ($('#volume-curve-factor span').text() == "Less (-20)") {SESSION.json['volcurvefac'] = 76;}
	else if ($('#volume-curve-factor span').text() == "Less (-30)") {SESSION.json['volcurvefac'] = 86;}
	else if ($('#volume-curve-factor span').text() == "More (+06)") {SESSION.json['volcurvefac'] = 50;}
	else if ($('#volume-curve-factor span').text() == "More (+12)") {SESSION.json['volcurvefac'] = 44;}
	else if ($('#volume-curve-factor span').text() == "More (+18)") {SESSION.json['volcurvefac'] = 38;}
	SESSION.json['volmaxpct'] = $('#volume-max-percent').val();

	// device settings
	if (UI.pcm5122Dev === true) {
		var analogVol = $('#pcm5122-analog-vol span').text() === '0 dB' ? '100' : '0'; // Analog volume
		var analogPbb = $('#pcm5122-analog-pbb span').text() === '.8 dB' ? '100' : '0'; // Analog playback boost
		var digFilter = $('#pcm5122-filter-name span').text(); // Digital interpolation filter
		var settings = analogVol + ',' + analogPbb + ',' + digFilter;
	
		// update sql
		if (settings != UI.devSettings) {
			var result = sendMoodeCmd('POST', 'updaudiodev', {'name': SESSION.json['i2sdevice'], 'settings': settings});
		}
	}
	...
	var result = sendMoodeCmd('POST', 'updcfgengine',
		{'volwarning': SESSION.json['volwarning'],
		 ...
		 'pldisp': SESSION.json['pldisp'],		
		 'xtagdisp': SESSION.json['xtagdisp'],		
		 'rsmaftersps': SESSION.json['rsmaftersps'],		
		 'volcurve': SESSION.json['volcurve'],		
		 'volcurvefac': SESSION.json['volcurvefac'],		
		 'volmaxpct': SESSION.json['volmaxpct'],		
	...
	if (themeChange == true) {
		...
		//$.post('moode.php', {syscmd: SESSION.json['themecolor'].toLowerCase()});
...
$('body').on('click', '.dropdown-menu .custom-select a', function() {
	...
	} else if ($(this).data('cmd') == 'resume-aftersps-yn') {
		$('#resume-aftersps span').text($(this).text());
	...
	} else if ($(this).data('cmd') == 'pcm5122-filter-name') {
		$('#pcm5122-filter-name span').text($(this).text());
	} else if ($(this).data('cmd') == 'pcm5122-analog-vol') {
		$('#pcm5122-analog-vol span').text($(this).text());
	} else if ($(this).data('cmd') == 'pcm5122-analog-pbb') {
		$('#pcm5122-analog-pbb span').text($(this).text());
